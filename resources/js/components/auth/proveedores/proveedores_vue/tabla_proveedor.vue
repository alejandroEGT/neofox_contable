      ...
      <template v-slot:body="tabla">
        <q-tr :props="tabla">
          ...
          <q-td key="opciones" :props="tabla">
            <q-toggle
              v-model="tabla.row.estado_id"
              color="primary"
              keep-color
              @input="cambiarEstado(tabla.row.id, tabla.row.estado_id)"
            >
              <q-tooltip anchor="bottom middle" self="top middle" :offset="[10, 10]">Cambiar Estado</q-tooltip>
            </q-toggle>
            <q-btn
              flat
